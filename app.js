const pesquisar = () => {
  ...
  setTimeout(() => {
    ...
    if (campoPesquisa === "") {
      ...
      section.innerHTML = "<p>Por favor, digite o nome de uma moto ou marca.</p>";
    ...
    const dadosFiltrados = dados.filter(dado => {
      const texto = `${dado.titulo} ${dado.descricao} ${dado.marca} ${dado.tags}`.toLowerCase();
...
document.getElementById('campo-pesquisa').addEventListener('keydown', (event) => {
  if (event.key === 'Enter') {
    pesquisar();
  }
});
